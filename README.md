...
| **Asset Type** | Market Position |
| **Control Record** | Market Position Management Plan |
...
| POST | `/v1/market-position-management-plan/initiate` | Initiate |
| GET | `/v1/market-position-management-plan` | Retrieve (list) |
| GET | `/v1/market-position-management-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/market-position-management-plan/{crId}/update` | Update |
| PUT | `/v1/market-position-management-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/market-position-management-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-position-management:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-position-management ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-position-management` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
